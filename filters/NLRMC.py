...
def NLRMC(data, lambda_val=3, max_rank=2, max_iters=100000):
    """
    Parameters:
        lambda_val (float, optional): Regularization strength (higher = smoother result).
        max_rank (int, optional): Maximum rank of the filled matrix.
    """
    if not max_rank:
        max_rank = round(2*data.shape[1]/3)
    # Create and apply SoftImpute model
    imputer = SoftImpute(
        shrinkage_value=lambda_val,
        max_rank=max_rank,
        max_iters=max_iters,
        verbose=False
    )
...
def routine(table, delta):
    ...
    for i in range(0,(table.shape[0] - table.shape[0] % delta), delta):
        out.iloc[i:i+delta,:] = NLRMC(table.iloc[i:i+delta,:].values)
    if table.shape[0] % delta != 0:
        out.iloc[table.shape[0]-delta:table.shape[0],:] = NLRMC(table.iloc[table.shape[0]-delta:table.shape[0],:].values)
